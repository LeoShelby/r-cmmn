...
export default function CustomCmmnRules(eventBus) {
  RuleProvider.call(this, eventBus);
}

inherits(CustomCmmnRules, RuleProvider);

CustomCmmnRules.$inject = [ 'eventBus' ];
...
CustomCmmnRules.prototype.init = function() {
    ...
    this.addRule('connection.create', HIGH_PRIORITY, function(context) {
        ...
        var allowed = canConnect(source,target);
    ...
    function canConnect(source,target){
        ...
        if (isCaseFileItem(source) && isAlternativeCaseFileItem(target)) {
            if(notHasAnyFileItemConnected(target) && !getRecoveryAttribute(source)){
                return {
                    type: 'cmmn:Connection',
                };
            }
        ...
        if(isAlternativeCaseFileItem(source) && isAlternativeCaseFileItem(target)){
            if(notHasAnyFileItemConnected(target) && getChainValue(source) < 3 ){
                return {
                    type: 'cmmn:Connection',
                };
            }
        ...
        if(isMilestone(source) && isAlternativeMilestone(target)) {
            if(notHasAnyMilestoneConnected(target)){
                return {
                    type: 'cmmn:Connection',
                };
            }
        ...
        if(isAlternativeMilestone(source) && isAlternativeMilestone(target)){
            if(notHasAnyMilestoneConnected(target) && getChainValue(source) < 3){
                return {
                    type: 'cmmn:Connection',
                };
            }
    ...
    function notHasAnyFileItemConnected(element){
    ...
    function notHasAnyMilestoneConnected(element){
        var out = element.outgoing;
        for (let i=0; i< out.length; i++) {
          if(out[i].businessObject.cmmnElementRef.$type == 'cmmn:Connection'){
