...
var svgRemove = require('tiny-svg').remove;
...
export default class CustomCmmnRenderer extends BaseRenderer {
  constructor(eventBus, styles, pathMap) {
  ...
  canRender(element) {
    return is(element, 'cmmn:AlternativeCaseFileItem') && !element.labelTarget;;
  }

  drawShape(parentNode, element) {

    var pathData = this.pathMap.getScaledPath('DATA_OBJECT_PATH', {
        xScaleFactor: 1,
        yScaleFactor: 1,
...
CustomCmmnRenderer.$inject = [ 'eventBus', 'styles', 'pathMap' ];
...
function CustomCmmnRenderer(eventBus, styles, pathMap, customPathMap) {
  ...
  //-------------------------------------------
  function isAlternativeMilestoneLabel(element) {
    return is(getDefinition(element), 'cmmn:AlternativeMilestone');
  }

  function isAlternativeCaseFileItemLabel(element) {
  ...
  function renderEmbeddedLabel(p, element, align) {
    ...
    //-----------------------------------------------
    if(!name && isAlternativeMilestoneLabel(element)){
      name = 'X';
  ...
  function renderExpandedStageLabel(p, element, align) {
    ...
    //-----------------------------------------------
    if(!name && isRecoveryStage(element)){
      name = 'X';
      textbox = renderLabel(p, name, {
        box: element,
    ...
    //-----------------------------------------------
      ...
      textbox = renderLabel(p, name, { box: element, align: align, padding: 5 });
    }
  ...
  function renderExternalLabel(parentGfx, element) {
    ...
    if(!name && isAlternativeCaseFileItemLabel(element)){
      name = 'X';
      element.hidden = false;
      return renderLabel(parentGfx, name, { box: box, style: { fontSize: '16px', fontWeight: 'bold' } });
    }
    //-----------------------------------------------
  ...
  var handlers = {
    ...
    //----------------------------------------------------------------
    // RECOVERY STAGE
    'cmmn:RecoveryStage': function(p, element, attrs) {
    ...
    'cmmn:Milestone': function(p, element, attrs) {
      ...
      attachCriticalityValue(p,element);
      return rect;
    },

    //-------------------------------------------------------------

    'cmmn:AlternativeMilestone': function(p, element, attrs) {
    ...
    'cmmn:ErrorEventListener': function(parentGfx, element, attrs) {
      var circle = renderer('cmmn:EventListener')(parentGfx, element, attrs);

      var pathData = customPathMap.getScaledPath('EVENT_ERROR', {
        xScaleFactor: 1.1,
        yScaleFactor: 1.1,
        containerWidth: element.width,
        containerHeight: element.height,
    ...
    'cmmn:CaseFileItem': function(p, element, attrs) {
      ...
      var res =  drawPath(p, pathData, { fill: 'white' });
    ...
    'cmmn:AlternativeCaseFileItem': function(p, element, attrs) {
      ...
      var res = drawPath(p, pathData, attrs);
    ...
    'cmmn:Connection': function(p, element, attrs) {
    ...
    'ChainMarker': function(p, element, position) {
      ...
      // position parameter is used to pass the chain_value
      ...
      drawPath(p, path, { strokeWidth: 1, stroke: 'blue', fill: 'blue' });
    },

    'CriticalityMarker0': function(p, element, position) {

      var x,y;

      if(isCaseFileItem(element) || isAlternativeCaseFileItem(element)){
        x = -0.9;
        y = -0.3;
      }
      //Milestone
      ...
      var path = customPathMap.getScaledPath('CRITICALITY_NONE', {
        xScaleFactor: 1,
        yScaleFactor: 1,
        containerWidth: element.width,
        containerHeight: element.height,
      ...
      drawPath(p, path, { strokeWidth: 1, stroke: 'black', fill: 'black' });
    },

    'CriticalityMarker1': function(p, element, position) {
      ...
      var path = customPathMap.getScaledPath('CRITICALITY_CRITICAL', {
        xScaleFactor: 1,
        yScaleFactor: 1,
        containerWidth: element.width,
        containerHeight: element.height,
    ...
    'CriticalityMarker0.2': function(p, element, position) {
      ...
      var path = customPathMap.getScaledPath('CRITICALITY_LOW', {
        xScaleFactor: 1,
        yScaleFactor: 1,
        containerWidth: element.width,
        containerHeight: element.height,
    ...
    'CriticalityMarker0.5': function(p, element, position) {
      ...
      var path = customPathMap.getScaledPath('CRITICALITY_MEDIUM', {
        xScaleFactor: 1,
        yScaleFactor: 1,
        containerWidth: element.width,
        containerHeight: element.height,
    ...
    'CriticalityMarker0.8': function(p, element, position) {
      ...
      var path = customPathMap.getScaledPath('CRITICALITY_HIGH', {
        xScaleFactor: 1,
        yScaleFactor: 1,
        containerWidth: element.width,
        containerHeight: element.height,
    ...
    'label': function(parentGfx, element) {
      ...
      */
      ...
      svgAttr(textElement, {
        transform: 'translate(' + (-1 * textBBox.x) + ',4)'
      });
  ...
  function attachCriticalityValue(p, element) {
    
    
    var criticality_value = 0;

    var old_value = getCriticalityValue(element);

    if(!old_value){
      setCriticalityValue(element,criticality_value);
    }
    else{
      criticality_value = old_value;
    }

    
    drawMarker('CriticalityMarker' + criticality_value, p, element, 0);
 
  }


  function attachChainValue(p, element) {
    
    var chain_value = getChainValue(element);
    
    if(chain_value) drawMarker('ChainMarker', p, element, chain_value);
  ...
  function isAlternativeCaseFileItem(element){
    return element.type == 'cmmn:AlternativeCaseFileItem';
  }
  ...
  function setCriticalityValue(element, value){
    element.businessObject.definitionRef.criticalityValue = value;
  }

  function isSourceCaseFileitem(element){
    return element.businessObject.cmmnElementRef.sourceRef.$type == 'cmmn:CaseFileItem';
  }

  function isSourceMilestone(element){
    if(element.businessObject.cmmnElementRef.sourceRef.definitionRef){
      if(element.businessObject.cmmnElementRef.sourceRef.definitionRef.$type == 'cmmn:Milestone'){
          return true;
      }
    return false;
    }
  ...
  function isTargetTask(element){
    if(element.businessObject.cmmnElementRef.targetRef.definitionRef){
      if(element.businessObject.cmmnElementRef.targetRef.definitionRef.$type == 'cmmn:Task'){
          return true;
      }
    return false;
    }
  ...
  function isTargetEventListener(element){
    if(element.businessObject.cmmnElementRef.targetRef.definitionRef){
      if(element.businessObject.cmmnElementRef.targetRef.definitionRef.$type == 'cmmn:EventListener'){
          return true;
      }
    return false;
    }
  ...
  function isTargetAlternativeCaseFileitem(element){
    return element.businessObject.cmmnElementRef.targetRef.$type == 'cmmn:AlternativeCaseFileItem';
  }

  function isSourceAlternativeCaseFileitem(element){
    return element.businessObject.cmmnElementRef.sourceRef.$type == 'cmmn:AlternativeCaseFileItem';
  }

  function isTargetAlternativeMilestone(element){
    if(element.businessObject.cmmnElementRef.targetRef.definitionRef){
  ...
  function isSourceAlternativeMilestone(element){
    if(element.businessObject.cmmnElementRef.sourceRef.definitionRef){
      if(element.businessObject.cmmnElementRef.sourceRef.definitionRef.$type == 'cmmn:AlternativeMilestone'){
          return true;
      }
    return false;
    }
...
inherits(CustomCmmnRenderer, BaseRenderer);

CustomCmmnRenderer.$inject = [ 'eventBus', 'styles', 'pathMap', 'customPathMap' ];

module.exports = CustomCmmnRenderer;
